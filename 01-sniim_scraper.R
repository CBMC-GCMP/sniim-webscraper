...
setwd("/Users/fabiofavoretto/CBMC Dropbox/Fabio/scraper-sniim")
...
source("R/nytnyt.R") # Function to create random time interval to not saturate web queries


load("data/dictionary_sp_name.Rds")

# Input days --------------------------------------------------------------

sysdate <- Sys.Date()

day1 <- as.numeric(day(sysdate))
day2 <- as.numeric(day(sysdate))
month <- as.numeric(month(sysdate))
year <- as.numeric(year(sysdate))          
...
## Save to file
data.table::fwrite(results, file = "data/last_prices.csv")
